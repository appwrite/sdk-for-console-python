...
class Oauth2(Service):
    ...
    def logout(
        ...
        id_token_hint : Optional[str]
            ID Token previously issued to the app, used as proof of the logout request. Required to end the session; signature and issuer are validated while expiry is ignored.
        logout_hint : Optional[str]
            Hint about the user that is logging out. Accepted for OIDC compatibility.
        client_id : Optional[str]
            OAuth2 client ID. When both `client_id` and `id_token_hint` are provided, they must identify the same app.
        post_logout_redirect_uri : Optional[str]
            URI to redirect the user to after logout. Must exactly match a URI registered in the app's `postLogoutRedirectUris`.
        state : Optional[str]
            Opaque value passed back unchanged in the `state` query param of the post-logout redirect.
        ui_locales : Optional[str]
            Preferred languages for any logout UI, as space-separated BCP47 tags. Accepted for OIDC compatibility.
        Returns
        -------
        Dict[str, Any]
        ...
        api_path = '/oauth2/{project_id}/logout'
        api_params = {}
        api_path = api_path.replace('{project_id}', str(self._normalize_value(self.client.get_config('project'))))
