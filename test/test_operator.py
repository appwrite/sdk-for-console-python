...
from appwrite_console.operator import Operator, Condition
...
class TestOperatorMethods(unittest.TestCase):
    ...
    def test_array_append(self):
        self.assertEqual(
            Operator.array_append(['item1', 'item2']), '{"method":"arrayAppend","values":["item1","item2"]}'
        )

    def test_array_prepend(self):
        self.assertEqual(
            Operator.array_prepend(['first', 'second']), '{"method":"arrayPrepend","values":["first","second"]}'
        )
    ...
    def test_array_intersect(self):
        self.assertEqual(
            Operator.array_intersect(['a', 'b', 'c']), '{"method":"arrayIntersect","values":["a","b","c"]}'
        )
    ...
    def test_array_filter(self):
        self.assertEqual(
            Operator.array_filter(Condition.EQUAL, 'test'), '{"method":"arrayFilter","values":["equal","test"]}'
        )
